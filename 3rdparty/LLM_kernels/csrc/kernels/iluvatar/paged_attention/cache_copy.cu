...
__global__ void ProcessKvListKernel(void** kv_list, size_t layer_num, size_t block_num, size_t block_size) {
  ...
  if (layer_idx < layer_num && block_idx < block_num) {
    // IXCC：void* 上做指针算术 NVCC 容忍但 IXCC 拒绝；显式 cast 成 char* 再偏移。
    kv_list[layer_idx * block_num + block_idx] =
        static_cast<char*>(kv_list[block_idx]) + layer_idx * block_size / layer_num;
  }
}
