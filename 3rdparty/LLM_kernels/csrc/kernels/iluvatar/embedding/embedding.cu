...
// Iluvatar 适配版本：与 nvidia/embedding.cu 内容一致，仅做编译器兼容性微调：
//   1) IXCC 的 `min/max` 不会在多个整型重载之间隐式选择，这里改用 std::min。
// 上游 nvidia 版做任何更新时，请同步 diff 这两点修改。
#include "embedding.h"
#include <algorithm>
...
void LookupFusedEmbeddingWithCSRInputs(T* output_hidden_units, const T* embedding_table, const T* pos_table,
  ...
  dim3 grid(std::min(static_cast<int32_t>(batch_size), DEFAULT_CUDA_GPU_DEVICE_MAX_BLOCKS_NUM), tokens_stride);
  dim3 block(std::min(hidden_units, static_cast<uint32_t>(DEFAULT_CUDA_BLOCK_THREADS_NUM)));
